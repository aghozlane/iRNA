...
#            i+=1    
    ...
    def getPredplotvect(self,listobj):
        ...
        result_vect=[]
        for obj in listobj:
            result_vect+=[self.meanArray(obj.sensitivity), self.meanArray(obj.ppv)]
        ...
        for obj in listobj:
            ...
            try:
                with open(output, 'wb') as f:
                    ...
                    f.write("Mean interaction length : %d nt\n"%np.mean(interaction))
                    f.write("Maximum interaction length : %d nt\n"%np.max(interaction))
                    f.write("Minimum interaction length : %d nt\n"%np.min(interaction))
            except IOError: sys.exit("Error : the program can not open %s"%output)     
